...
function mostrarStates(array) {
    ...
    let result = estados.filter((item, index) => {
...
function filtroState(array, state) {
    ...
    let estadoFiltrado = array.filter((estado) => estado.state == state);
...
function renderTablaMiembros(array, id) {
    ...
    if (filtroParty(filtroState(miembros, select.value), partySel()) != "") {
        array.forEach((miem) => {
            tablaMiembros.innerHTML += `
        <tr>
        <td><a class="text-light " href="${
          miem.url
        }" target="_blank">${miem.last_name} ${miem.first_name} ${
              miem.middle_name ? miem.middle_name : " "
            }</a> </td>
        <td>${miem.party}</td>
        <td>${miem.state}</td>
        ...
        tablaMiembros.innerHTML += `<td colspan="5"><div class = "alert alert-danger text-center"
        role = "alert" >
...
function update() {
    renderTablaMiembros(
        filtroParty(filtroState(miembros, select.value), partySel()).sort((a, b) =>
